  ...
  module Task
    module Sheet683Point2kw
      ...
      module InstanceMethods
        def process_683_point_2kw
          self.process do |xlsx|
            power_unit = PowerUnit.create(unit: '683.2kW')
            ...
            power_unit_items.each do |power_unit_item|
              ...
              (power_sub_unit_start_row_number..total_rows).to_a.each do |row_index|
                ...
                if single_units.include?(power_unit_item)
                  ...
                  first_column_name = "unit_#{power_sub_unit_row[index + 1].downcase}"
                  sub_unit_measurement.send("#{first_column_name}=", row[index + 1])
